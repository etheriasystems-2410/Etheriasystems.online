...
import { SpiralGlyph } from '../components/Glyphs';
...
const TarotSpreads: React.FC = () => {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      scrollTl
        .fromTo(bg, { scale: 1.12, opacity: 0.8 }, { scale: 1, opacity: 1, ease: 'none' }, 0)
        .fromTo(glyph, { opacity: 0, rotate: 25, scale: 0.9 }, { opacity: 1, rotate: 0, scale: 1, ease: 'power2.out' }, 0)
      ...
      scrollTl
        .to(bg, { scale: 1.05, opacity: 0.4, ease: 'none' }, 0.7)
        .to(glyph, { opacity: 0, scale: 0.96, ease: 'power2.in' }, 0.7)
  ...
  return (
    <section ref={sectionRef} className="section-pinned z-[120]">
      <div ref={bgRef} className="absolute inset-0 z-[1]" style={{ opacity: 0.8 }}>
        <img src="/tarot-spreads.jpg" alt="Tarot Spreads" className="w-full h-full object-cover" />
      </div>
      <div className="absolute inset-0 bg-black/50 z-[2]" />
      <div className="vignette z-[3]" />
      <div className="absolute top-1/4 right-1/3 orb orb-1 z-[4]" />
      <div className="absolute bottom-1/3 left-1/4 orb orb-2 z-[4]" />
      <div className="relative z-[5] w-full h-full flex flex-col items-center justify-center px-8">
        ...
        <div ref={glyphRef} className="mb-6 text-foreground drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)] spiral-rotate" style={{ opacity: 0 }}>
          <SpiralGlyph size={100} />
        </div>
        ...
        <p ref={bodyRef} className="mt-6 text-center max-w-[56ch] text-foreground font-inter text-[clamp(14px,1.3vw,18px)] leading-relaxed tracking-wide drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)] bg-black/40 px-6 py-4 rounded-sm" style={{ opacity: 0 }}>
          From the simple Three-Card spread to the masterful Celtic Cross. Learn how position creates meaning, how cards speak to each other, and how to weave a narrative from the symbols before you.
        </p>
      </div>
    </section>
  );
