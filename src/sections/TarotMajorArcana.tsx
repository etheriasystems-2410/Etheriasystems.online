...
const TarotMajorArcana: React.FC = () => {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      scrollTl
        .fromTo(bg, { scale: 1.12, x: '6vw', opacity: 0.8 }, { scale: 1, x: 0, opacity: 1, ease: 'none' }, 0)
        .fromTo(glyph, { opacity: 0, y: '10vh', scale: 0.9 }, { opacity: 1, y: 0, scale: 1, ease: 'power2.out' }, 0)
  ...
  return (
    <section ref={sectionRef} id="tarot-major-arcana" className="section-pinned z-[105]">
      <div ref={bgRef} className="absolute inset-0 z-[1]" style={{ opacity: 0.8 }}>
        <img src="/tarot-major-arcana.jpg" alt="Major Arcana" className="w-full h-full object-cover" />
      </div>
      ...
      <div className="relative z-[5] w-full h-full flex flex-col items-center justify-center px-8">
        ...
        <p ref={bodyRef} className="mt-6 text-center max-w-[56ch] text-foreground font-inter text-[clamp(14px,1.3vw,18px)] leading-relaxed tracking-wide drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)] bg-black/40 px-6 py-4 rounded-sm" style={{ opacity: 0 }}>
          The Fool's Journey begins. Master the 22 archetypal cards that map the soul's evolution—from the innocence of The Fool to the completion of The World. Understand the deep symbolism that has guided seekers for centuries.
        </p>
      </div>
    </section>
  );
